...
fn parse_line(
    ...
    let parse: fn(&str) -> Option<(String, u16)> =
        if is_ipv6 { parse_ipv6_addr } else { parse_ipv4_addr };
    ...
    Some(Connection { proto, local_ip, local_port, remote_ip, remote_port, state, pid, process })
}
...
fn build_pid_map() -> HashMap<u64, (u32, String)> {
    ...
    for entry in proc_dir.flatten() {
        ...
        let Ok(fd_dir) = fs::read_dir(format!("/proc/{pid}/fd")) else { continue };
