...
pub fn inspect(raw: &[u8]) -> DeepPacket {
    ...
    if src_mac == [0xff; 6] { anomalies.push("Source MAC is broadcast (invalid)".into()); }
...
fn parse_arp(data: &[u8], anomalies: &mut Vec<String>) -> Option<ArpDetail> {
    ...
    let op_str = match op { 1 => "Request", 2 => "Reply", _ => "?" };
...
fn parse_ipv4(data: &[u8], _raw: &[u8], dp: &mut DeepPacket, anomalies: &mut Vec<String>) {
    ...
    if version != 4 { anomalies.push(format!("Unexpected IP version: {version}")); }
    if ihl < 20     { anomalies.push("IPv4 IHL < 20 (malformed)".into()); return; }
    if data.len() < ihl { anomalies.push("IPv4 header longer than frame".into()); return; }
    ...
    if frag_offset > 0 || dp.ip_flag_mf {
        ...
        anomalies.push(format!("Fragmented packet (offset={frag_offset}, MF={})", dp.ip_flag_mf));
    }
    ...
    if data[8] == 0 { anomalies.push("TTL=0 (invalid)".into()); }
    if data[8] == 1 { anomalies.push("TTL=1 (will expire on next hop)".into()); }

    // Check source IP sanity
    let src = dp.ip_src.unwrap();
    if src.is_unspecified()  { anomalies.push("Source IP is 0.0.0.0".into()); }
    if src.is_broadcast()    { anomalies.push("Source IP is broadcast (spoofed?)".into()); }
...
fn parse_tcp(data: &[u8], dp: &mut DeepPacket, anomalies: &mut Vec<String>) {
    ...
    if syn && fin   { anomalies.push("TCP SYN+FIN set simultaneously (scan/malformed)".into()); }
    if syn && rst   { anomalies.push("TCP SYN+RST set simultaneously (malformed)".into()); }
    if !syn && !ack_f && !fin && !rst && !psh { anomalies.push("TCP no flags set (NULL scan)".into()); }
    if syn && psh   { anomalies.push("TCP SYN+PSH unusual".into()); }
    if window == 0 && syn { anomalies.push("TCP SYN with zero window (malformed)".into()); }

    if hdr_len < 20 { anomalies.push("TCP data offset < 20 (malformed)".into()); return; }
    if data.len() < hdr_len { anomalies.push("TCP header longer than segment".into()); return; }
        ...
        match opts[i] {
            ...
            5 => {  // SACK blocks
                ...
                while j + 8 <= i + len {
                    ...
                    let right = u32::from_be_bytes([opts[j+4], opts[j+5], opts[j+6], opts[j+7]]);
            ...
            _ => {
                ...
                let len = opts[i+1] as usize;
                if len < 2 {
                    anomalies.push(format!("TCP option kind={} has length<2 (malformed)", opts[i]));
                    break;
                }
                i += len;
            }
...
fn parse_udp(data: &[u8], dp: &mut DeepPacket, anomalies: &mut Vec<String>) {
    ...
    if udp_len < 8 { anomalies.push("UDP length < 8 (malformed)".into()); }
...
fn detect_app_tcp(src: u16, dst: u16, payload: &[u8], dp: &mut DeepPacket) {
    ...
    match dst.max(src) {
        80 | 8080 | 8000 => { detect_http(payload, dp); }
        443 | 8443        => detect_tls(payload, dp),
        22                => { dp.app_proto = Some("SSH".into()); }
        25 | 587 | 465    => detect_smtp(payload, dp),
        110 | 995         => { dp.app_proto = Some("POP3".into()); }
        143 | 993         => { dp.app_proto = Some("IMAP".into()); }
        21                => { dp.app_proto = Some("FTP Control".into()); }
        23                => { dp.app_proto = Some("Telnet".into()); }
        3306              => { dp.app_proto = Some("MySQL".into()); }
        5432              => { dp.app_proto = Some("PostgreSQL".into()); }
        6379              => { dp.app_proto = Some("Redis".into()); }
        27017             => { dp.app_proto = Some("MongoDB".into()); }
        3389              => { dp.app_proto = Some("RDP".into()); }
        179               => { dp.app_proto = Some("BGP".into()); }
        _                 => {
...
fn detect_app_udp(src: u16, dst: u16, payload: &[u8], dp: &mut DeepPacket) {
    match dst.max(src) {
        ...
        123         => { dp.app_proto = Some("NTP".into()); }
        161 | 162   => { dp.app_proto = Some("SNMP".into()); }
        500 | 4500  => { dp.app_proto = Some("IKE/IPsec".into()); }
        514         => { dp.app_proto = Some("Syslog".into()); }
        1900        => { dp.app_proto = Some("SSDP".into()); }
        5353        => detect_dns(payload, dp),  // mDNS
        4789        => { dp.app_proto = Some("VXLAN".into()); }
        6081        => { dp.app_proto = Some("Geneve".into()); }
        _           => {}
    }
}
        ...
        if text.starts_with(m) {
            ...
            for line in text.lines().skip(1) {
                if line.to_lowercase().starts_with("host:") {
                    dp.app_detail.push(format!("Host     : {}", line[5..].trim()));
                }
                if line.to_lowercase().starts_with("user-agent:") {
                    dp.app_detail.push(format!("UserAgent: {}", line[11..].trim()));
                }
                if line.to_lowercase().starts_with("content-type:") {
                    dp.app_detail.push(format!("Content  : {}", line[13..].trim()));
                }
    ...
    if text.starts_with("HTTP/") {
        ...
        for line in text.lines().skip(1) {
            ...
            if line.to_lowercase().starts_with("server:") {
                dp.app_detail.push(format!("Server   : {}", line[7..].trim()));
            }
            if line.is_empty() { break; }
        }
        return true;
...
fn detect_tls(payload: &[u8], dp: &mut DeepPacket) {
    ...
    if ver_major != 3 || ver_minor > 4 { return; }
...
fn extract_sni(payload: &[u8]) -> Option<String> {
    ...
    let mut pos = 43;
    if payload.len() <= pos { return None; }
...
fn detect_dns(payload: &[u8], dp: &mut DeepPacket) {
    ...
    dp.app_detail.push(format!("Type     : {}  OpCode={}", if is_resp { "Response" } else { "Query" }, opcode));
    dp.app_detail.push(format!("Flags    : RD={} RA={} AA={}", rd as u8, ra as u8, aa as u8));
    if is_resp {
        dp.app_detail.push(format!("RCode    : {}", dns_rcode(rcode)));
    ...
    if qdcount > 0 {
        if let Some((name, end)) = dns_parse_name(payload, 12) {
            if end + 4 <= payload.len() {
                ...
                dp.app_detail.push(format!("Question : {} {} CLASS={}", name, dns_type(qtype), qclass));
            } else {
...
fn dns_rcode(code: u16) -> &'static str {
    match code {
        ...
        3 => "NXDOMAIN", 4 => "NOTIMP", 5 => "REFUSED", _ => "?",
    }
}
...
fn dns_type(t: u16) -> &'static str {
    match t {
        ...
        15=>"MX", 16=>"TXT", 28=>"AAAA", 33=>"SRV", 255=>"ANY", _=>"?",
    }
}
...
fn dns_parse_name(data: &[u8], start: usize) -> Option<(String, usize)> {
    ...
    let mut hops   = 0u8;
    loop {
        if hops > 20 || pos >= data.len() { return None; }
        let len = data[pos] as usize;
        if len == 0 { pos += 1; break; }
        if len & 0xC0 == 0xC0 {
            if pos + 1 >= data.len() { return None; }
            let ptr = ((len & 0x3F) << 8) | (data[pos + 1] as usize);
            pos = ptr; hops += 1; continue;
        }
        pos += 1;
...
fn detect_dhcp(payload: &[u8], dp: &mut DeepPacket) {
    if payload.len() < 236 { return; }
    let msg_type = payload[0];
    let op = match msg_type { 1 => "BOOTREQUEST", 2 => "BOOTREPLY", _ => "?" };
    ...
    if !ciaddr.is_unspecified() { dp.app_detail.push(format!("ClientIP : {ciaddr}")); }
    if !yiaddr.is_unspecified() { dp.app_detail.push(format!("YourIP   : {yiaddr}")); }
    // Client MAC
    ...
    if payload.len() >= 240 && &payload[236..240] == b"\x63\x82\x53\x63" {
        ...
        while i < opts.len() {
            match opts[i] {
                ...
                _ => {
                    if i + 1 >= opts.len() { break; }
                    i += 2 + opts[i+1] as usize; continue;
                }
            }
        }
    }
...
fn detect_smtp(payload: &[u8], dp: &mut DeepPacket) {
    ...
    if let Ok(text) = std::str::from_utf8(&payload[..payload.len().min(256)]) {
        ...
        if !first.is_empty() { dp.app_detail.push(format!("Banner   : {first}")); }
    }
}
...
fn decode_tcp_flags(flags: u16) -> String {
    ...
    if flags & 0x0040 != 0 { parts.push("ECE"); }
    if flags & 0x0080 != 0 { parts.push("CWR"); }
...
fn icmp_describe(t: u8, code: u8) -> String {
    match t {
        ...
        3  => format!("Dest Unreachable — {}", match code {
            ...
            13=>"Comm admin prohibited", _=>"?",
        }),
        4  => "Source Quench (deprecated)".into(),
        5  => format!("Redirect — {}", match code { 0=>"Net", 1=>"Host", 2=>"Net+TOS", 3=>"Host+TOS", _=>"?" }),
        8  => "Echo Request".into(),
        9  => "Router Advertisement".into(),
        10 => "Router Solicitation".into(),
        11 => format!("Time Exceeded — {}", match code { 0=>"TTL in transit", 1=>"Fragment reassembly", _=>"?" }),
...
fn ether_type_name(et: u16) -> &'static str {
    match et {
        ...
        0x8100 => "VLAN (802.1Q)",  0x88a8 => "QinQ (802.1ad)",
        0x8847 => "MPLS Unicast",   0x8848 => "MPLS Multicast",
        0x0842 => "Wake-on-LAN",    0x88cc => "LLDP",
        0x88e5 => "MACsec",         0x9000 => "Loopback",
        _ => "Unknown",
    }
}
...
impl DeepPacket {
    ...
    pub fn one_liner(&self) -> String {
        let proto = if self.tcp_src_port.is_some()  { "TCP" }
            else if self.udp_src_port.is_some()     { "UDP" }
            else if self.icmp_type.is_some()        { "ICMP" }
            else if self.arp.is_some()              { "ARP" }
            else                                    { "ETH" };
        ...
        let flags_s = self.tcp_flags_str.as_deref()
            .filter(|s| *s != "[none]")
            .map(|s| format!(" {s}"))
            .unwrap_or_default();
        ...
        let ttl_s  = self.ip_ttl.map(|t| format!("  TTL={t}")).unwrap_or_default();
        let win_s  = if self.tcp_window == Some(0) { "  [ZERO-WIN]" } else { "" };
        let icmp_s = self.icmp_type_str.as_deref()
            .map(|s| format!("  {s}"))
            .unwrap_or_default();
        let vlan_s = if self.vlan_tags.is_empty() { String::new() } else {
            let ids: Vec<_> = self.vlan_tags.iter().map(|t| t.id.to_string()).collect();
            format!("  VLAN={}", ids.join("→"))
        };

        format!("[{proto}{flags_s}]  {src} → {dst}{svc}{ttl_s}{vlan_s}{icmp_s}{win_s}  ({} bytes)",
            self.frame_len)
    }
    ...
    fn diagnose_tcp(&self, out: &mut Vec<String>) {
        ...
        if syn && !ack {
            ...
            if self.tcp_mss.is_none() {
                out.push("No MSS option in SYN — unusual; may cause fragmentation on this path".into());
            }
        } else if syn && ack {
            ...
            out.push(format!("TCP data push — {} bytes of application payload", self.tcp_payload_len));
        ...
        if let Some(mss) = self.tcp_mss {
            if mss < 536 {
                out.push(format!("Very small MSS={mss} — path likely has a tunnel or strict MTU constraint"));
            } else if mss == 1452 {
    ...
    fn diagnose_arp(&self, out: &mut Vec<String>) {
        ...
        match arp.operation {
            "Request" => out.push(format!("ARP who-has {}? — broadcast from {} ({})",
                arp.target_ip, arp.sender_ip, arp.sender_mac)),
            "Reply"   => out.push(format!("ARP reply: {} is at {} — L2 neighbor cache updated",
    ...
    fn diagnose_misc(&self, out: &mut Vec<String>) {
        ...
        if self.ip_flag_mf || self.ip_fragment.unwrap_or(0) > 0 {
            out.push("Fragmented IP — full decode only possible after all fragments are reassembled".into());
        }
...
fn quick_svc(port: u16) -> Option<&'static str> {
    Some(match port {
        ...
        123 => "NTP", 143 => "IMAP", 179 => "BGP", 443 => "HTTPS",
...
pub fn hex_dump(data: &[u8], max_bytes: usize) -> Vec<String> {
    ...
    for (i, chunk) in data.chunks(16).enumerate() {
        ...
        while hex_padded.len() < 48 { hex_padded.push(' '); }
...
impl DeepPacket {
    fn empty(frame_len: usize) -> Self {
        DeepPacket {
            frame_len,
            eth_src: String::new(), eth_dst: String::new(),
            eth_vendor_src: None,   eth_vendor_dst: None,
            vlan_tags: Vec::new(),
            ether_type: 0, ether_type_name: "",
            arp: None,
            ip_version: None, ip_src: None, ip_dst: None,
            ip_hdr_len: None, ip_total_len: None, ip_id: None,
            ip_ttl: None, ip_proto: None, ip_proto_name: None,
            ip_dscp: None, ip_ecn: None,
            ip_flag_df: false, ip_flag_mf: false, ip_fragment: None,
            tcp_src_port: None, tcp_dst_port: None,
            tcp_seq: None, tcp_ack: None,
            tcp_flags: None, tcp_flags_str: None,
            tcp_window: None, tcp_urgent: None,
            tcp_hdr_len: None, tcp_payload_len: 0,
            tcp_mss: None, tcp_window_scale: None,
            tcp_sack_permitted: false, tcp_sack_blocks: Vec::new(),
            tcp_timestamp: None,
            udp_src_port: None, udp_dst_port: None,
            udp_len: None, udp_checksum: None, udp_payload_len: 0,
            icmp_type: None, icmp_code: None, icmp_type_str: None,
            icmp_id: None, icmp_seq: None, icmp_checksum: None,
            app_proto: None, app_detail: Vec::new(),
            payload: Vec::new(),
            anomalies: Vec::new(),
        }
    }
